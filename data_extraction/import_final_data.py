...
def clear_tables():
    ...
    for table in tables:
        try:
            ...
            print(f"  ⚠️  {table}: 已清空或表为空")


def import_themes_final():
    """导入题材数据（最终版本）"""
    print("\n📥 导入题材数据 (themes final)...")
...
def import_elements_final():
    """导入元素数据（最终版本）"""
    print("\n📥 导入爆款元素 (theme_elements final)...")
    ...
    for element in elements:
        try:
            ...
            if success <= 10 or success % 5 == 0:
                print(f"  ✅ ({success}) {element['name'][:30]}...")
        except Exception as e:
            pass
...
def import_examples_final():
    """导入案例数据（最终版本）"""
    print("\n📥 导入标杆案例 (theme_examples final)...")
    ...
    for example in examples:
        try:
            ...
            success += 1
            if success <= 10 or success % 5 == 0:
                print(f"  ✅ ({success}) {example['title'][:30]}...")
        except Exception as e:
            pass
...
def import_hooks_final():
    """导入钩子模板（最终版本）"""
    print("\n📥 导入钩子模板 (hook_templates final)...")
...
def main():
    ...
    # 导入数据
    total = 0
    total += import_themes_final()
    total += import_elements_final()
    total += import_examples_final()
    total += import_hooks_final()
