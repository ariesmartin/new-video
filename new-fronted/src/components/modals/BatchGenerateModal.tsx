...
export function BatchGenerateModal({ isOpen, onClose }: BatchGenerateModalProps) {
  ...
  const [mobileTab, setMobileTab] = useState<'settings' | 'queue'>('settings');
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-4xl h-[85vh] rounded-xl overflow-hidden flex flex-col bg-surface border border-border"
      ...
      >
        ...
        <div className="flex sm:hidden border-b border-border">
          <button
            className={`flex-1 py-2 text-sm font-medium ${mobileTab === 'settings' ? 'text-primary border-b-2 border-primary' : 'text-text-secondary'}`}
            onClick={() => setMobileTab('settings')}
          >
            生成参数
          </button>
          <button
            className={`flex-1 py-2 text-sm font-medium ${mobileTab === 'queue' ? 'text-primary border-b-2 border-primary' : 'text-text-secondary'}`}
            onClick={() => setMobileTab('queue')}
          >
            生成队列 ({tasks.filter(t => t.status === 'completed').length}/{tasks.length})
          </button>
        </div>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className={`
            ...
            ${mobileTab === 'settings' ? 'block' : 'hidden sm:block'}
          `}>
            <h3 className="text-xs font-medium uppercase tracking-wider mb-4 text-text-tertiary hidden sm:block">
              生成参数
            </h3>

            {/* 画风风格 */}
            <div className="mb-4">
              <label className="text-xs block mb-2 text-gray-400">
                画风风格
              </label>
              <select
                value={selectedStyle}
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 px-3 rounded-lg text-sm transition-colors ${
                      resolution === res
                        ...
                        : 'border hover:bg-white/5 border-border'
                    }`}
                  >
                    {res}高清
                  </button>
            ...
            <div className="space-y-2">
              {!isGenerating ? (
                ...
                <Button 
                  className="w-full flex items-center justify-center gap-2 bg-status-yellow text-black"
          ...
          <div className={`
            ...
            ${mobileTab === 'queue' ? 'block' : 'hidden sm:block'}
          `}>
            <h3 className="text-xs font-medium uppercase tracking-wider mb-4 text-gray-500 hidden sm:block">
              生成队列
            </h3>
            ...
            <div className="space-y-3">
              {tasks.map((task) => (
                <div
                  key={task.id}
                  className="p-3 rounded-lg border bg-gray-900 border-border"
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span
                        className="text-xs font-medium px-2 py-0.5 rounded bg-surface text-gray-100"
                      >
                        #{task.cardNumber}
                      </span>
