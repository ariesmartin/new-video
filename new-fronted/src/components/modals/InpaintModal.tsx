  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-surface border border-border flex flex-col">
        ...
        {/* 工具栏 */}
        <div className="flex flex-wrap items-center gap-2 p-2 rounded-lg mb-4 bg-gray-900 overflow-x-auto">
          <button
          ...
          <button
            onClick={clearCanvas}
            className="p-2 rounded hover:bg-white/10 transition-colors text-text-secondary"
            title="清空"
          >
            <Trash2 size={18} />
          </button>
          ...
          <div className="flex items-center gap-2 ml-auto w-full sm:w-auto mt-2 sm:mt-0 justify-end">
            ...
            />
            <span className="text-xs w-6 text-text-secondary">
              {brushSize}
            </span>
          </div>
        </div>
        ...
        {/* 画布区域 */}
        <div 
          className="flex-1 overflow-auto flex items-center justify-center rounded-lg bg-elevated min-h-0"
        >
          {imageUrl ? (
            <canvas
              ref={canvasRef}
        ...
        {/* 提示词和操作 */}
        <div className="mt-4 flex flex-col sm:flex-row items-end gap-3 sm:gap-4 flex-shrink-0">
